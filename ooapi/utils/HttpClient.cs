...
namespace org.openoces.ooapi.utils
{
    ...
    {
        static internal byte[] Download(string location)
        {
            if (location == null)
            {
        ...
        static byte[] ReadBytesFromUri(Uri uri)
        {
            ...
            using (var response = request.GetResponse())
            {
                ...
                {
                    return ReadBytesFromStream(responseStream);
                }
            }
        }

        static byte[] ReadBytesFromStream(Stream responseStream)
        {
