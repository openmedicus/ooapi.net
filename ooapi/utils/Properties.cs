...
namespace org.openoces.ooapi.utils
{
    ...
    {
        static readonly Dictionary<string, string> _properties = new Dictionary<string, string>
      {
        // LDAP servers
        ...
        private static int _httpCrlCacheTimeout = 0;
        private static int _ldapCrlCacheTimeout = 0;


        public static int GetHttpCrlCacheTimeout()
        {
            if (_httpCrlCacheTimeout == 0)
            {
                _httpCrlCacheTimeout = Properties.IsDefined("crl.cache.timeout.http") ? int.Parse(Properties.Get("crl.cache.timeout.http")) : CRL_CACHE_DEFAULT_TIMEOUT;
        ...
        public static int GetLdapCrlCacheTimeout()
        {
            if (_ldapCrlCacheTimeout == 0)
            {
                _ldapCrlCacheTimeout = Properties.IsDefined("crl.cache.timeout.ldap") ? int.Parse(Properties.Get("crl.cache.timeout.ldap")) : CRL_CACHE_DEFAULT_TIMEOUT;
                // timeout can not be less than two minutes
        ...
        public static string icaCNToURL(string icaCN)
        {
            return Get(icaCN);
        }
    }
