        ...
        static void AddIssuerCertificateIfNeeded(IList<X509Certificate2> certificates)
        {
            if (certificates.Count == 1)
            {
                ...
                if (certificate.Issuer.ToUpper().Contains("TRUST2408"))
                {
                    ...
                    catch (InvalidCaIssuerUrlException ex)
                    {
                        //certificate is missing an extension, so determine how to alternatively fetch ica certificate
                        if (serviceprovider.ServiceProviderSetup.CurrentChecker.GetType() == typeof(FullCrlRevocationChecker) || serviceprovider.ServiceProviderSetup.CurrentChecker.GetType() == typeof(OcspCertificateRevocationChecker))
                        {
                            String icaHttpURL = Properties.icaCNToURL(certificate.Issuer.ToUpper());
                            var icaCertificate = new X509Certificate2(HttpClient.Download(icaHttpURL));
                            certificates.Add(icaCertificate);
                        }
                        else
                        {
                            X509Certificate2 icaCert = new LdapCrlDownloader().DownloadCertificate(Environments.getOces_II_Environment(), certificate.Issuer.ToUpper());
